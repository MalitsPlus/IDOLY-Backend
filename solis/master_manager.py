...
def one_task(name: str, key: str, url: str):
    ...
    enc_data = send_request(requests.get, url, verify=False).content
...
def generate_data(master_tag: transp.MasterTag):
    ...
    console.succeed(f"New masterdata version '{master_tag.version}'.")
    set_cache("masterVersion", master_tag.version)
